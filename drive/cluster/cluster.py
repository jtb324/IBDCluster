...
class Networks:
    ...
    def gather_cluster_info(
        ...
        for clst_id in cluster_ids:
            print(self.graph.vs)
            print(self.graph.vs[0])
            self.clusters[clst_id] = {}
            self.clusters[clst_id]["memberID"] = [
                self.graph.vs[c]["name"]
                for c, v in enumerate(random_walk_clusters.membership)
                if v == clst_id
            ]
            print(cluster_ids.membership)

            self.clusters[clst_id]["member"] = [
                c for c, v in enumerate(random_walk_clusters.membership) if v == clst_id
            ]
            clst_edge_n = (
            ...
            self.clusters[clst_id]["true_positive_edge"] = list(
                filter(
                    ...
                    self.graph.get_eids(
                        pairs=list(
                            itertools.combinations(self.clusters[clst_id]["member"], 2)
                        ),
                        directed=False,
                        error=False,
                    ),
            ...
            for mem in self.clusters[clst_id]["member"]:
                all_edge = all_edge.union(set(random_walk_clusters.incident(mem)))

            self.clusters[clst_id]["false_negative_edge"] = list(
                all_edge.difference(
                    list(
                        random_walk_clusters.get_eids(
                            pairs=list(
            ...
            if (
                self.check_times < self.max_recheck
